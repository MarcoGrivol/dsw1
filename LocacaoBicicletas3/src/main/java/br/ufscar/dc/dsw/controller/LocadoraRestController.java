...
public class LocadoraRestController {
	...
	private void parse(Locadora locadora, JSONObject json) {
		...
		locadora.setRole("USER");
		locadora.setSenha(encoder.encode((String) json.get("nome")));
	}
	...
	public ResponseEntity<List<Locadora>> listaPorCidade(@PathVariable("nome") String nomeCidade) {
		List<Locadora> lista = service.buscarTodasPorCidade(nomeCidade);
		if (lista.isEmpty()) {
			return ResponseEntity.notFound().build();
		}
		return ResponseEntity.ok(lista);
	...
	public ResponseEntity<String> remove(@PathVariable("id") long id) {
		...
		if (locadora == null) {
			return ResponseEntity.notFound().build();
		} else if (!locacaoService.buscarLocacaoPorLocadora(locadora).isEmpty()) {
			return ResponseEntity.badRequest().body("Erro! Locadora possui uma locação.");
		} else {
			service.excluir(id);
			return ResponseEntity.accepted().body("Locadora removida com sucesso.");
		}
	}
}
